export const completereplicaddocs = `Skip to main content
Replicad Logo
replicad
Documentation
API
...
# Spectre monotile stamp

A friend of mine is into ceramics and pottery, and he asked me to help him design a stamp to apply a spectre monotile pattern to his pieces. This is the result.

The stamp can be configured for different sizes, with the side of the tile configurable as well.

A system of lock and key is used to help the removal of the stamp from the clay.
...
My landlord hired by painter to paint the ceiling of my flat. They did not remove ceiling lamps, so when we wanted to install a new lamp, but we could see where the paint was not done.

So I designed this rosette to hide the bad paint job. Also, this allows me to have more cable hidden (and therefore not cut it too short).

I also added two screw slits to fix the rosette to the ceiling. I configured it to fit with the holes from the previous lamp.

I printed the rosette in white PLA, with tree supports. The supports were easy to remove, and the rosette looks great.
...
#A training wheel for my son's bike

We bought a second hand bike for my son and the training wheel broke. These wheels are just a plastic cylinder with a metal rod that attaches to the bike.

The rod was not broken, so I just needed to replace the wheel. I used a lot of wall layers and a gyroid infill.
...
# A fixture for a square USB hub

I wanted to fix my i-tech USB hub to the bottom of my desk. I designed this bracket to do just that. The bracket is parametric and can be adjusted to fit any square thing you want to fix to the bottom of your desk. You will need to use 4 screws (and you can tweak the parameters just for that).
...
I have some flexible pipes that I need for brewing beer. The diameter of the pipes did not fit, so I designed this adapter to make them fit. Printed in TPU it does a great job of sealing the connection.
...
# Holder for decorations

I need a very specific shape to hold some decorations on a pole. This is what I came up with. Probably only useful to me.
...
# A shopping cart coin

Many shopping carts in Switzerland require a 2CHF coin to unlock. Personally, I often find myself without spare change.
...
# Hook to hang on furniture

This is a fairly simple object that is quite useful as a parametric model. The parameter that is probably the most useful is the hangerHeight.
...
# A strainer for your sink

I needed some kind of strainer to avoid that all the stuff that gets into the sink goes down the drain (and stucks it). So I made this model to generate drain strainers.
...
This model was the original model that lead to the more complex HSW web app. You can also watch this video, for the conference talk that prompted me to build it.
...
This was meant for use with boardgame tiles - to store them in an efficient manner.
...
In order to test the lid of my box for pencils, I printed without interior. My sister in law enjoyed using the small box for storing her earrings - and so I made of version of this box without interior - just to print lidded boxes.
...
This is a set of experiments to make boxes to store tokens for Heroes of Normandy. They work (and have been printed), but I am not 100% sure this is the best approach to the problem.
...
# Hinged box for crayons or pencils

This model has been created to keep some erasable pencils for my son (of the brand Super Petit). But I made it generic enough so that it can be used for any type of crayon.

Note that the design was originally thought of to be able to print it in place, but printing the cover was not great. The cover snaps in easily if printed separately as well (and it looks great as a two colors piece).
...
# A bracket to hold my scale

I needed a convenient spot to store my scale where it would be easily accessible. However, when kept in cupboards or drawers, it frequently drained its batteries quickly.

This holder is designed to be affixed with adhesive to securely hold the scale in place. You’ll need to print two of them!
...
# A gridfinity tray generator

This was designed mostly as a modelling exercise. Configure the structure you want and it spits out a printable 3D model.
...
# Corners to stack cardboard boxes

If you need more context on why you would want to use this, read it on my blog.
...
The code is listed below. For the piramid I demonstrated three different approaches, one exact and two approximations that are nearly indistinguishable from the exact version. You can also try the link to the workbench. Since creating the link in to the workbench I enhanced the creation of the "halfpipe", using only one ellipse() statement.
...
.ellipse(-bDim,0,bDim/2,bDim/2,0,false,true)
...
# Created somthing similar to the YouTube logo

Here a funny experiment to demonstrate how boolean actions on drawings and shapes can help to create quite complicated shapes without needing complex tools to generate tangent curves.
...
let youTubeLogo = circleRight.intersect(circleLeft)
.intersect(circleBottom)
.intersect(circleTop)
youTubeLogo = youTubeLogo.sketchOnPlane("XY").extrude(20)
let bigBall = makeSphere(400).translate([0,0,-380])
youTubeLogo = youTubeLogo.intersect(bigBall)
.fillet(13.4,(e)=>e.inDirection("Z")).fillet(5)
...
# Inspired by Matt Parker, https://www.youtube.com/watch?v=a_fTA4UeFls, I created the following model of the One World Trade Center. The model illustrates how to use the makePolygon and makeSolid functions of Replicad. Modelling this shape was not straightforward. You can tell by the code that I needed some tricks to make it work. The code can be improved to create antiprisms of other regular polygons. This would require a more generic approach to define the rotation of the top plane versus the base plane and to define the faces on the top and bottom. For now I am happy with the result.
...
# Created the clamp from Model Mania 2023

To show that you can create quite complicated shapes using only a combination of basic shapes, I created the model as used for Solidworks Model Mania 2023. The result is:
...
# Created the knob from Solidworks Modelmania 2006
...
In CascadeStudio I used an offset inwards to create the cutout between the two bearings. Although Replicad seems to contain an offset function, this seems only to work on faces. It is not clear how to use this function.
In CascadeStudio I used a fillet in the sketch of the sideview (the Y shape). In Replicad there seems to be no fillet function in the Sketcher class, so I filleted the 3D shape after extruding the sideview.
